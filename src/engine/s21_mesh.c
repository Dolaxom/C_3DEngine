...
void copy_polygons(mesh_t original_mesh) {
  for (int i = 0; i < original_mesh.count_of_polygons; i++) {
    for (int j = 0; j < original_mesh.polygons[i].count_of_points; j++) {
      original_mesh.polygons_copy[i].points[j] =
          original_mesh.polygons[i].points[j];
    }
  }
}
...
mesh_t parse_obj_file(char* path_to_file, int* error_code) {
  ...
  *error_code = S21_OK;
  ...
  if ((file = fopen(path_to_file, "r")) != NULL) {
    ...
    if (result_mesh.v_points == NULL || result_mesh.v_points_copy == NULL) {
      exit(S21_MALLOC);
    }
    while (fgets(str_from_file, 64, file)) {
      ...
      if (str_from_file[0] == 'v') {
        ...
        result_mesh.v_points =
            realloc(result_mesh.v_points, points_count * sizeof(vector));
        result_mesh.v_points_copy =
            realloc(result_mesh.v_points_copy, points_count * sizeof(vector));
        if (result_mesh.v_points == NULL || result_mesh.v_points_copy == NULL) {
          exit(S21_REALLOC);
        }
        ...
        while (str_from_file[i] != '\0' && str_from_file[i] != '\n') {
          while (str_from_file[i] != ' ' && str_from_file[i] != '\0' &&
                 str_from_file[i] != '\n') {
          ...
          points_tmp =
              realloc(points_tmp, (current_points_count + 1) * sizeof(int));
          if (points_tmp == NULL) {
            exit(S21_REALLOC);
          }
        ...
        int offset_array = 1;
        result_mesh.polygons =
            realloc(result_mesh.polygons, polygons_count * sizeof(polygons_t));
        result_mesh.polygons_copy = realloc(
            result_mesh.polygons_copy, polygons_count * sizeof(polygons_t));
        if (result_mesh.polygons == NULL || result_mesh.polygons_copy == NULL) {
          exit(S21_REALLOC);
        }

        result_mesh.polygons[polygons_count - offset_array].points =
            (vector*)malloc(current_points_count * sizeof(vector));
        result_mesh.polygons_copy[polygons_count - offset_array].points =
            (vector*)malloc(current_points_count * sizeof(vector));
        if (result_mesh.polygons[polygons_count - offset_array].points ==
                NULL ||
            result_mesh.polygons_copy[polygons_count - offset_array].points ==
                NULL) {
          exit(S21_MALLOC);
        ...
        for (int z = 0; z < current_points_count; z++) {
          result_mesh.polygons[polygons_count - offset_array].points[z] =
              result_mesh.v_points[points_tmp[z] - offset_array];
        }
        result_mesh.queue =
            realloc(result_mesh.queue,
                    (queue_count + current_points_count) * sizeof(size_t));

        for (int k = 0; k < current_points_count; k++) {
          result_mesh.queue[queue_count] = points_tmp[k] - offset_array;
          queue_count++;
        }
        result_mesh.polygons[polygons_count - offset_array].count_of_points =
            current_points_count;
        result_mesh.polygons_copy[polygons_count - offset_array]
            .count_of_points = current_points_count;
