...
OpenGLWidget::OpenGLWidget(QWidget *parent) : QOpenGLWidget(parent) {
  mesh.count_of_points = 0;
  color_bg = QColor(0, 0, 0);
  color_vert = QColor(1, 1, 1);
  color_edge = QColor(1, 1, 1);

  connect(this, SIGNAL(autorotValueChanged(bool)), this,
          SLOT(autoRotateLaunch()));
}

// PUBLIC

void OpenGLWidget::clearMesh() {
  if (mesh.count_of_points == 0) {
    return;
  }
  if (mesh.polygons_copy != NULL) {
    free(mesh.polygons_copy);
  }
  if (mesh.polygons != NULL) {
    free(mesh.polygons);
  }

  if (mesh.v_points != NULL) {
    free(mesh.v_points);
  }
  if (mesh.v_points_copy != NULL) {
    free(mesh.v_points_copy);
  }
  if (mesh.queue != NULL) {
    free(mesh.queue);
  }
...
void OpenGLWidget::setMeshpath(QString new_meshpath) {
  if (new_meshpath != current_meshpath) {
    ...
    if (meshpath) {
      ...
      strncpy(meshpath, new_meshpath.toStdString().c_str(),
              (new_meshpath.length() + 1));
      initMesh(meshpath);
      update();
      free(meshpath);
    } else {
...
void OpenGLWidget::setColors(QColor new_bgcolor, QColor new_vertcolor,
...
int OpenGLWidget::getErrcode() { return errcode; }

int OpenGLWidget::getPolygonsCount() { return mesh.count_of_polygons; }

int OpenGLWidget::getPointsCount() { return mesh.count_of_points; }
...
void OpenGLWidget::updateProjection() {
  if (projection == 0) {  // perspective
    ...
    glFrustum(left_value, right_value, top_value, down_value, near_dist,
              far_dist);
...
void OpenGLWidget::initMesh(char *path_to_mesh) {
  errcode = 0;
  mesh_t new_mesh = parse_obj_file(path_to_mesh, &errcode);
